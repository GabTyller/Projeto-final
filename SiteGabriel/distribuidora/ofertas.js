...
const apiUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Ofertas?key=${apiKey}`;
...
async function fetchBebidas() {
    try {
        ...
        bebidas = data.values.slice(1).map(row => ({
            imageUrl: row[0],     // Coluna A
            name: row[1],         // Coluna B
            oldPrice: row[2],     // Coluna C
            newPrice: row[3]      // Coluna D
        }));
...
function renderPage(page) {
    ...
    const itemsToDisplay = bebidas.slice(startIndex, endIndex);
    ...
    itemsToDisplay.forEach(bebida => {
        ...
        bebidaItem.innerHTML = `
            ...
            <div class="price-container">
                <span class="old-price" style="text-decoration: line-through; color: red;">${bebida.oldPrice}</span>
                <span class="new-price">${bebida.newPrice}</span>
            </div>
